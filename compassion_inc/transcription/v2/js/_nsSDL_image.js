...
SDL.image = (function () {
    ...
    var bEnhanced = false;	
	var bExpanded = true;
	var preferAutoFit = false;
	var contrastRate = 300;
    ...
    var zoom = function (bApplyExisting) {
		...
		if (preferAutoFit) {					
			zoom  = (bIsZoomed ? Math.max(fitToHeight, fitToWidth) * 2 : Math.min(fitToWidth,fitToHeight));
		}
	...
	var setContrastRate = function(rate) {		
		contrastRate = rate;
	}
    ...
    return {
		...
		setContrastRate		: setContrastRate,
		init				: init
    };
    // --- expose public ---
