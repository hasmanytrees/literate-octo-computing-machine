...
SDL.transcription = (function () {
    ...
    var getSelectedTask = function () {
        ...
        var passedTaskId = getTaskParameter();

        if (passedTaskId != null) {
            iSelectedTaskId = passedTaskId;
            return passedTaskId;
        } else {
    ...
    var init = function () {
        ...
        SDL.shared.executeWSRequest('getCheckAccess',
            function (success, response) {
                ...
                SDL.transcription_io.load(iSelectedTaskId, function () {
                    $('body').show(); // add spinner
                    ready = true;
                    bindEvents();
                });
    ...
    var bindEvents = function () {
        ...
        $('textarea').on('focus', function () {
            updateProgress();
            //save(); todo: should not be needed with on-change debounce
        });
    }
    ...
    var bindEvents_keyboard = function () {
        ...
        Mousetrap.bind('ctrl+down', function (e) {
            return keyboardEvent(enumKeyboardShortcut.TAB_NEXT);
        });
        Mousetrap.bind('ctrl+up', function (e) {
            return keyboardEvent(enumKeyboardShortcut.TAB_PREV);
        });
        Mousetrap.bind('ctrl+left', function (e) {
            return keyboardEvent(enumKeyboardShortcut.PAGE_PREV);
        });
        Mousetrap.bind('ctrl+right', function (e) {
            return keyboardEvent(enumKeyboardShortcut.PAGE_NEXT);
        });
        Mousetrap.bind('ctrl+shift+a', function (e) {
            return keyboardEvent(enumKeyboardShortcut.PAGE_ALL);
        });
        Mousetrap.bind('ctrl+enter', function (e) {
            return keyboardEvent(enumKeyboardShortcut.CONFIRM);
        });

        // image options
        Mousetrap.bind('ctrl+e', function (e) {
            return keyboardEvent(enumKeyboardShortcut.IMG_ENHANCE);
        });
        Mousetrap.bind('ctrl+z', function (e) {
            return keyboardEvent(enumKeyboardShortcut.IMG_ZOOM);
        });
        Mousetrap.bind('ctrl+x', function (e) {
            return keyboardEvent(enumKeyboardShortcut.IMG_EXPAND);
        });

        // image navigation
        Mousetrap.bind('alt+up', function (e) {
            return keyboardEvent(enumKeyboardShortcut.SCROLL_UP);
        });
        Mousetrap.bind('alt+down', function (e) {
            return keyboardEvent(enumKeyboardShortcut.SCROLL_DOWN);
        });
    ...
    var save = function (callback) {
        ...
        if (!ready) {
            showCriticalError('UI was not correctly initialized!');
            return;
        }
        ...
        var mapFields = $('textarea').map(function () {
            return $(this).val();
        });

        if (mapFields == null || mapFields.length == 0) {
            alert("CRITICAL ERROR: Update fields contain no values. Cannot save values!");
            return;
        }

        /*if (inSave) {
            console.log("Aborted; save in-progress!");
            return;
        }
        */

        console.log("Updating server values: " + mapFields);
        ...
        */
        ...
        if (!arraysEqual(lastSave, mapFields)) {

            SDL.transcription_io.save(iSelectedTaskId, mapFields, function (success) {
                lastSave = mapFields;
                if (callback != null) callback(success);
            });
    ...
    var bindEvents_viewer = function () {
        ...
        $('#imageViewer').on('mouseleave', function (e) {
            $('.menu').hide();
        });
        $('#imageScroller').on('mousedown', function (e) {
            e.preventDefault();
        }, false);
        $('#imageScroller').on('dblclick', function (e) {
            checkClickType(enumClickType.DOUBLE, function () {
                $('#btn_enhance').click();
            });
        });
    ...
    var initUIJqueryUI = function () {
        ...
        $('textarea').on('input propertychange', _.debounce(function () {
            save();
        }, 5000)); // handles typed input and paste
        $('textarea').on('input propertychange', _.debounce(function () {
            setStatusIcon($(this));
        }, 350)); // handles typed input and paste
    ...
    function getTaskParameter() {

        var taskId = '&taskId=';
        var iTaskIdx = location.href.indexOf(taskId);
        var iEndIdx = location.href.indexOf('&', iTaskIdx + 1);
        ...
        return location.href.substring(iTaskIdx + taskId.length, iEndIdx);
    }
